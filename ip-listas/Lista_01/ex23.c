...
int main () {
    ...
    int n_vendas, l1 = 0, l2 = 0, l3 = 0, ml = 0, mv = 0;

    while (scanf("%u %f %f %d", &codigo, &preco_compra, &preco_venda, &n_vendas) != EOF){

        if (lucro(preco_compra, preco_venda) < 10) l1 += 1;
        if (lucro(preco_compra, preco_venda) >= 10 && lucro(preco_compra, preco_venda) <= 20 ) l2 += 1;
        if (lucro(preco_compra, preco_venda) > 20) l3 += 1;
